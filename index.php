...
<body>
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="col-lg-8">
        <?php 
          $post_result = mysqli_query($conn, "SELECT posts.id,posts.image,posts.title,posts.body,posts.created_date,posts.modified_date,posts.user_id,users.name,users.image as user_photo FROM posts,users WHERE posts.user_id=users.id ORDER BY created_date DESC"); 
        ...
        ?>
          ...
          <h1 class="mt-4" id="clear_underline">
            <a href="post.php?id=<?php echo $postrow['id']?>" id="clear_underline"><?php echo $postrow['title'] ?></a>
          </h1>
          ...
          ?>    
            <?php 
              $commentresult =mysqli_query($conn ,"select c.post_id,c.body,u.name,u.image from users u join comments c on u.id=c.user_id join posts p on p.id=c.post_id"); 
